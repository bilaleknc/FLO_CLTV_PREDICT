...
# Flo verisini okutalım.Dataframe’in kopyasını oluşturalım.
df_ = pd.read_csv("C:/Users/Victus/PycharmProjects/pythonProject/venv/datasets/flo_data_20k.csv")
df = df_.copy()
...
# Not: cltv hesaplanırken frequency değerleri integer olması gerekmektedir.Bu nedenle alt ve üst limitlerini round() ile yuvarlayalım.
...
# 1.Veri setindeki en son alışverişin yapıldığı tarihten 2 gün sonrasını analiz tarihi olarak alalım.
...
# 6 ay içerisinde müşterilerden beklenen satın almaları tahmin edelim ve exp_sales_6_month olarak cltv dataframe'ine ekleyiniz.
cltv_df["exp_sales_6_month"] = bgf.predict(4 * 6,
                                           cltv_df['frequency'],
                                           cltv_df['recency_cltv_weekly'],
                                           cltv_df['T_weekly'])

# 3. ve 6.aydaki en çok satın alım gerçekleştirecek 10 kişiyi inceleyelim. Fark var mı?
...
# 1. 6 aylık CLTV'ye göre tüm müşterilerinizi 4 gruba (segmente) ayırınız ve grup isimlerini veri setine ekleyelim.
# cltv_segment ismi ile atayalım.
